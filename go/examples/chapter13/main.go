...
func main() {
	...
	right := render.NewCone()
	right.SetMinimum(0)
	right.SetMaximum(1)
	right.SetTransform(render.Translation(1.5, 0.5, -0.5).Multiply(render.Scaling(0.5, 0.5, 0.5)).Multiply(render.RotationX(math.Pi)))
	...
	rightCyl := render.NewCylinder()
	rightCyl.SetMinimum(0)
	rightCyl.SetMaximum(1)
	rightCyl.SetTransform(render.Translation(0.5, 0.5, -0.5).Multiply(render.Scaling(0.5, 0.5, 0.5)))
	rightCylMaterial := render.NewMaterial(render.NewColor(1, 1, 1), 0.0, 0, 0, 0.3, 1, 3, 200, nil)
	rightCyl.SetMaterial(rightCylMaterial)
	world.AddObject(rightCyl)
